...
def lookup(vv,s):
    """
    vector lookup - look up a vector from vector of vectors and interpolate 
    vv - vector of vectors
    s  - index (cyclic and float: can be floating point, can be larger than the length of vv, can be negtive)
    """
    n = len(vv)        # number of colors in color map
    fs = math.floor(s) # integer smaller than s
    v0 = vv[fs % n]
    v1 = vv[(fs+1) % n]
    r = s - fs         # fraction part of s
    return inter(v0, v1, r)
